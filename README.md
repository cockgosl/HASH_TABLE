...
Основные функции взаимодействия со списком: list_init(list*, type) - инициализация списка, создание головы(head), list_destroy(list*) - очистка памяти после работы, list_insert(list*, type) - вставка нового элемента после узла (head), list_dump(list*) - распечатка информации о текущем содержимом списка.

2. *Таблица*

Фактически массив списков. Функции: make_table(table_t*, char*, size_t , hash_ptr ) - на вход подаётся инициализированная таблица, указатель на готовый массив, возвращаемый функцией read_buffer(FILE* text, size_t amount*)(буферизует текст, делит его на слова, считает кол-во слов, записывает его в переменную amount), следующий параметр - amount(количество слов , которое должно помещаться в таблицу), затем хеш-функция, с помощью которой мы эту таблицу строим, возвращаемое значение - (enum) CORRECT(всё хорошо), INCORRECT(нет). Следующая функции - table_init(table_t*, size_t)(какую таблицу инициализируем + на сколько элементов), table_destroy(table_t*) - очищаем память после работы, table_dump(table_t*, FILE*) - распечатка данных таблицы для построения графика, search(table_t* , char* , hash_ptr) - в какой таблице ищем, какое слово ищем, в соответствии с каким хешом.
